#!/usr/bin/env python3
"""Create or update WeChat Official Account drafts from Markdown/URL articles."""
...
class WeChatClient:
    ...
    def _draft_text_candidates(
        ...
        if not title_candidates:
            title_candidates = ["未命名文章"]
        if not digest_candidates:
            digest_candidates = ["AI 资讯速览"]

        return safe_author, title_candidates, digest_candidates

    def _draft_article_payload(
        self,
        title: str,
        author: str,
        digest: str,
        ...
        if existing_article:
            preserved_source_url = str(existing_article.get("content_source_url") or "").strip()
            try:
                need_open_comment = int(existing_article.get("need_open_comment", need_open_comment))
            except (TypeError, ValueError):
                pass
            try:
                only_fans_can_comment = int(
                    existing_article.get("only_fans_can_comment", only_fans_can_comment)
                )
            except (TypeError, ValueError):
                pass

        final_source_url = (source_url or preserved_source_url).strip()[:200]
        return {
            "title": title,
            "author": author,
            "digest": digest,
            "content": content_html,
            "content_source_url": final_source_url,
    ...
    def add_draft(
        ...
        safe_author, title_candidates, digest_candidates = self._draft_text_candidates(
            title=title,
            author=author,
            digest=digest,
        )
    ...
    def get_draft(self, token: str, media_id: str) -> dict[str, Any]:
        data = self._post_json_utf8(
            DRAFT_GET_URL,
            {"access_token": token},
            {"media_id": media_id},
        ...
        if data.get("errcode", 0) != 0:
            raise WeChatPublishError(f"draft get failed: {data}")
        return data

    def get_draft_article(self, token: str, media_id: str, index: int) -> dict[str, Any]:
        data = self.get_draft(token, media_id)
        articles = data.get("news_item")
        if not isinstance(articles, list):
            raise WeChatPublishError("draft get failed: missing news_item")
        if index >= len(articles):
            raise WeChatPublishError(
                f"draft get failed: index {index} out of range, article_count={len(articles)}"
            )
        article = articles[index]
        if not isinstance(article, dict):
            raise WeChatPublishError(f"draft get failed: invalid article at index {index}")
        return article
        ...
        for safe_title in title_candidates:
            ...
            for safe_digest in digest_candidates:
                payload = {
                    "media_id": media_id,
                    "index": index,
                    "articles": self._draft_article_payload(
                        title=safe_title,
                        author=safe_author,
                        digest=safe_digest,
                        content_html=content_html,
                ...
                if errcode == 45004:
                    continue
                raise WeChatPublishError(f"draft update failed: {data}")

            if should_shorten_title:
                continue

        raise WeChatPublishError(f"draft update failed: {last_error}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--draft-media-id", default="", help="Update an existing draft instead of creating one")
    parser.add_argument("--draft-index", type=int, default=0, help="Article index inside an existing draft, default 0")
    parser.add_argument("--thumb-media-id", default="", help="Reuse an existing WeChat cover media_id")
...
def validate_args(args: argparse.Namespace) -> None:
    draft_media_id = (args.draft_media_id or "").strip()
    if args.draft_index < 0:
        raise RuntimeError("--draft-index 必须大于或等于 0")
    if args.draft_index and not draft_media_id:
        raise RuntimeError("--draft-index 需要配合 --draft-media-id 使用")
    if args.cover_image:
        cover_path = Path(args.cover_image).expanduser().resolve()
        if not cover_path.exists():
            raise RuntimeError(f"--cover-image 指定的文件不存在: {cover_path}")


def resolve_cover_strategy(args: argparse.Namespace, article: Article) -> str:
    if (args.thumb_media_id or "").strip():
        return "thumb-media-id"
    if args.cover_image:
        return "cover-image"
    if (args.draft_media_id or "").strip():
        return "existing-draft-thumb"
    if article.first_image_url.startswith(("http://", "https://")):
        return "auto-generate-or-first-image"
...
def main() -> None:
            ...
            json.dumps(
                {
                    "success": True,
                    "mode": "dry-run",
                    "operation": "update" if update_mode else "create",
                    "draft_media_id": draft_media_id if update_mode else "",
                    "draft_index": args.draft_index if update_mode else None,
                    "title": article.title,
                    "digest": article.digest,
                    "template": template,
                    "cover_strategy": resolve_cover_strategy(args, article),
                    "thumb_media_id": (args.thumb_media_id or "").strip(),
    ...
    existing_draft_article: dict[str, Any] | None = None
    if update_mode:
        existing_draft_article = client.get_draft_article(token, draft_media_id, args.draft_index)

    thumb_media_id = (args.thumb_media_id or "").strip()
    cover_image_path = Path(args.cover_image).expanduser().resolve() if args.cover_image else None
    if update_mode:
        if not thumb_media_id and cover_image_path and cover_image_path.exists():
            thumb_media_id = client.upload_image_from_path(token, cover_image_path)
        elif not thumb_media_id and existing_draft_article:
            thumb_media_id = str(existing_draft_article.get("thumb_media_id") or "").strip()
    else:
    ...
    if not thumb_media_id and update_mode:
        raise RuntimeError(
            "更新模式无法复用现有封面，请传入 --thumb-media-id 或 --cover-image 覆盖当前封面"
        )
    ...
    if update_mode:
        client.update_draft(
            ...
            thumb_media_id=thumb_media_id,
            existing_article=existing_draft_article,
        )
    else:
    ...
    if update_mode:
        result["draft_index"] = args.draft_index
